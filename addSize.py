	...
	for shape in shapes:
		...
		for filename in os.listdir(directory):
			if filename.endswith(".png"): 
				...
				fragSizes.append(cv2.contourArea(listOfPoints))
				...
				#descriptorArray = np.array(descriptorList)
				#print("collected descriptors: ",len(descriptorArray))
				continue
			else:
				continue
		...
		np.save("MPEG7dataset\\test\\extractions\\"+shape+"\\sizes",sizesArray)
